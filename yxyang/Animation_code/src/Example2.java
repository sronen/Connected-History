		...
		try{
			...
			try {
				...
				for (int j=minby;j<maxdy;j++){
					...
					for (int i=0; i < linkN; i++){
						if (j==edgesB[i]){
							int sumlet=linkList[i].let1+linkList[i].let2;
							int nodenum=0;
							if (linkList[i].node1!=1){
								nodenum=linkList[i].node1-1;
							}
							else{
								nodenum=linkList[i].node2-1;
							}
							if (sumlet<4){
								graph.setVertexAttribute(vertices[nodenum], "size", "0.4");
								graph.setVertexAttribute(vertices[nodenum], "color", "#ff0000");
							}
							else if (sumlet<40){
								graph.setVertexAttribute(vertices[nodenum], "size", "0.6");
								graph.setVertexAttribute(vertices[nodenum], "color", "#ff00ff");
							}
							else{
								graph.setVertexAttribute(vertices[nodenum], "size", "0.8");
								graph.setVertexAttribute(vertices[nodenum], "color", "#ffffff");
								graph.setVertexAttribute(vertices[nodenum], "label", expandNode[nodenum+1].name);
							}
